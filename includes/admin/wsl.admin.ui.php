...
function wsl_admin_main()
{
	...
	{
		wp_die( __( 'You do not have sufficient permissions to access this page.' , 'wordpress-social-login' ) );
	}
		...
		if( get_option( 'wsl_settings_development_mode_enabled' ) )
		{
			?>
				<div class="fade error wsl-error-dev-mode-on" style="margin: 4px 0 20px;">
					<p>
						<?php _e('<b>Warning:</b> You are now running WordPress Social Login with DEVELOPMENT MODE enabled. This mode is not intended for live websites as it might raise serious security risks', 'wordpress-social-login') ?>.
					</p>
					<p>
		...
		if( get_option( 'wsl_settings_debug_mode_enabled' ) )
		{
			?>
				<div class="fade updated wsl-error-debug-mode-on" style="margin: 4px 0 20px;">
					<p>
						<?php _e('<b>Note:</b> You are now running WordPress Social Login with DEBUG MODE enabled. This mode is not intended for live websites as it might add to loading time and store unnecessary data on your server', 'wordpress-social-login') ?>.
					</p>
					<p>
		...
		if( ! get_option( 'wsl_settings_migration_notice_is_read' ) )
		{
			?>
                <div class="fade updated" style="margin: 4px 0 20px; border-left-color: #ffc107;">
                    <p><b><?php _e("Breaking changes notice", 'wordpress-social-login') ?>:</b></p>
                    ...
                    <ol>
                        <li><?php _e("First, navigate to <b>WSL Settting > Networks</b> tab", 'wordpress-social-login') ?>,</li>
                        <li><?php _e("Next, refer to <b>Where do I get this info?</b> section for each enabled provider and copy its new auto generated callback url", 'wordpress-social-login') ?>,</li>
                        <li><?php _e("Finally, update that link on the provider developer website", 'wordpress-social-login') ?>.</li>
                    </ol>

                    <p><?php _e("It's also important to note that a number of providers are now made obsolete in WSL 3.0, while a few other were added, and cetain providers has changed their requirements", 'wordpress-social-login') ?>.</p>

					<p>
						<a class="button-secondary" style="background-color: #ffc107; border-color: #e3ab02; color: #fff;min-width: 95px;text-align: center;" href="options-general.php?page=wordpress-social-login&wslp=networks&wsldmn=1"><?php _e('Okey, dismiss this notice', 'wordpress-social-login') ?></a>
					</p>
				</div>
			<?php
		}
	...
	<table width="100%" border="0" style="margin:0;padding:0;">
		<tr>
			...
			<td width="" valign="top">
				...
				<ul style="margin-left:25px;">
					<li><?php _e('WSL is now fully migrated to Hybridauth 3.5', 'wordpress-social-login') ?></li>
					<li><?php _e("Reworked providers Callback URLs and it's a breaking change! See up top notice", 'wordpress-social-login') ?></li>
					<li><?php _e('Added serval new providers: Amazon, Discord and Spotify to name a few', 'wordpress-social-login') ?></li>
					<li><?php _e('Removal of a number of currently defunct identity providers ', 'wordpress-social-login') ?></li>
					<li><?php _e('Authentication widgets and forms design overhall', 'wordpress-social-login') ?></li>
					<li><?php _e('Various fixes and improvements', 'wordpress-social-login') ?></li>
				</ul>
			</td>
		</tr>
...
{
	add_options_page(
		__( 'WP Social Login', 'wordpress-social-login' ),
		__( 'WP Social Login', 'wordpress-social-login' ),
		'manage_options',
		'wordpress-social-login',
		'wsl_admin_main'
