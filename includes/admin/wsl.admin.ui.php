...
<div class="wsl-container">
			...
			if( $cache_enabled )
			{
				?>
					<div class="fade updated" style="margin: 4px 0 20px;">
						<p>
							<?php __e("<b>Note:</b> WSL has detected that you are using a caching plugin. If the saved changes didn't take effect immediately then you might need to empty the cache", 'wordpress-social-login') ?>.
						</p>
					</div>
				<?php
			}
		...
		if( get_option( 'wsl_settings_development_mode_enabled' ) )
		{
			?>
				<div class="fade error wsl-error-dev-mode-on" style="margin: 4px 0 20px;">
					<p>
						<?php __e('<b>Warning:</b> You are now running WordPress Social Login with DEVELOPMENT MODE enabled. This mode is not intend for live websites as it might raise serious security risks', 'wordpress-social-login') ?>.
					</p>
					<p>
						<a class="button-secondary" href="options-general.php?page=wordpress-social-login&wslp=tools#dev-mode"><?php __e('Change this mode', 'wordpress-social-login') ?></a>
						<a class="button-secondary" href="http://miled.github.io/wordpress-social-login/troubleshooting-advanced.html" target="_blank"><?php __e('Read about the development mode', 'wordpress-social-login') ?></a>
					</p>
				</div>
			<?php
		}
		...
		if( get_option( 'wsl_settings_debug_mode_enabled' ) )
		{
			?>
				<div class="fade updated wsl-error-debug-mode-on" style="margin: 4px 0 20px;">
					<p>
						<?php __e('<b>Note:</b> You are now running WordPress Social Login with DEBUG MODE enabled. This mode is not intend for live websites as it might add to loading time and store unnecessary data on your server', 'wordpress-social-login') ?>.
					</p>
					<p>
						<a class="button-secondary" href="options-general.php?page=wordpress-social-login&wslp=tools#debug-mode"><?php __e('Change this mode', 'wordpress-social-login') ?></a>
						<a class="button-secondary" href="options-general.php?page=wordpress-social-login&wslp=watchdog"><?php __e('View WSL logs', 'wordpress-social-login') ?></a>
						<a class="button-secondary" href="http://miled.github.io/wordpress-social-login/troubleshooting-advanced.html" target="_blank"><?php __e('Read about the debug mode', 'wordpress-social-login') ?></a>
					</p>
				</div>
			<?php
		}
	...
	<div class="alignright">
		<a style="font-size: 0.9em; text-decoration: none;" target="_blank" href="http://miled.github.io/wordpress-social-login/documentation.html"><?php __e('Docs', 'wordpress-social-login') ?></a> -
		<a style="font-size: 0.9em; text-decoration: none;" target="_blank" href="http://miled.github.io/wordpress-social-login/support.html"><?php __e('Support', 'wordpress-social-login') ?></a> -
		<a style="font-size: 0.9em; text-decoration: none;" target="_blank" href="https://github.com/miled/wordpress-social-login"><?php __e('Github', 'wordpress-social-login') ?></a>
	</div>

	<h1 <?php if( is_rtl() ) echo 'style="margin: 20px 0;"'; ?>>
		<?php __e( 'WordPress Social Login', 'wordpress-social-login' ) ?>
...
?>
<div id="wsl_div_warn">
	<h3 style="margin:0px;"><?php __e('Oops! We ran into an issue.', 'wordpress-social-login') ?></h3>

	<hr />

	<p>
		<?php __e('Unknown or Disabled <b>Component</b>! Check the list of enabled components or the typed URL', 'wordpress-social-login') ?> .
	</p>

	<p>
		<?php __e("If you believe you've found a problem with <b>WordPress Social Login</b>, be sure to let us know so we can fix it", 'wordpress-social-login') ?>.
	</p>

	<hr />

	<div>
		<a class="button-secondary" href="http://miled.github.io/wordpress-social-login/support.html" target="_blank"><?php __e( "Report as bug", 'wordpress-social-login' ) ?></a>
		<a class="button-primary" href="options-general.php?page=wordpress-social-login&wslp=components" style="float:<?php if( is_rtl() ) echo 'left'; else echo 'right'; ?>"><?php __e( "Check enabled components", 'wordpress-social-login' ) ?></a>
	</div>
</div>
<?php
	// HOOKABLE:
...
<div class="wsl-container">
		<div style="background: none repeat scroll 0 0 #fff;border: 1px solid #e5e5e5;box-shadow: 0 1px 1px rgba(0, 0, 0, 0.04);padding:20px;">
			...
			<p>
				<?php __e("The minimum server requirements are", 'wordpress-social-login') ?>:
			</p>

			<ul style="margin-left:60px;">
				<li><?php __e("PHP >= 5.2.0 installed", 'wordpress-social-login') ?></li>
				<li><?php __e("WSL Endpoint URLs reachable", 'wordpress-social-login') ?></li>
				<li><?php __e("PHP's default SESSION handling", 'wordpress-social-login') ?></li>
				<li><?php __e("PHP/CURL/SSL Extension enabled", 'wordpress-social-login') ?></li>
				<li><?php __e("PHP/JSON Extension enabled", 'wordpress-social-login') ?></li>
				<li><?php __e("PHP/REGISTER_GLOBALS Off", 'wordpress-social-login') ?></li>
				<li><?php __e("jQuery installed on WordPress backoffice", 'wordpress-social-login') ?></li>
			</ul>
		</div>
...
-->
<div id="wsl-w-panel">
	<a href="options-general.php?page=wordpress-social-login&wslp=<?php echo $wslp ?>&wsldwp=1" id="wsl-w-panel-dismiss" <?php if( is_rtl() ) echo 'style="left: 10px;right: auto;"'; ?>><?php __e("Dismiss", 'wordpress-social-login') ?></a>

	<table width="100%" border="0" style="margin:0;padding:0;">
		<tr>
			<td width="10" valign="top"></td>
			<td width="300" valign="top">
				<b style="font-size: 16px;"><?php __e("Welcome!", 'wordpress-social-login') ?></b>
				<p>
					<?php __e("If you are still new to WordPress Social Login, we have provided a few walkthroughs to get you started", 'wordpress-social-login') ?>.
				</p>
			</td>
			...
			<td width="260" valign="top">
				<br />
				<p>
					<b><?php __e("Get Started", 'wordpress-social-login') ?></b>
				</p>
				<ul style="margin-left:25px;">
					<li><a href="http://miled.github.io/wordpress-social-login/overview.html" target="_blank"><?php __e('Plugin Overview', 'wordpress-social-login') ?></a></li>
					<li><a href="http://miled.github.io/wordpress-social-login/networks.html" target="_blank"><?php __e('Setup and Configuration', 'wordpress-social-login') ?></a></li>
					<li><a href="http://miled.github.io/wordpress-social-login/widget.html" target="_blank"><?php __e('Customize WSL Widgets', 'wordpress-social-login') ?></a></li>
					<li><a href="http://miled.github.io/wordpress-social-login/userdata.html" target="_blank"><?php __e('Manage users and contacts', 'wordpress-social-login') ?></a></li>
					<li><a href="http://miled.github.io/wordpress-social-login/documentation.html" target="_blank"><?php __e('WSL Developer API', 'wordpress-social-login') ?></a></li>
				</ul>
			</td>
			<td width="" valign="top">
				<br />
				...
				<ul style="margin-left:25px;">
					<li><?php __e('WSL fully migrated to Hybridauth 3', 'wordpress-social-login') ?></li>
					<li><?php __e('Reworked providers Callback URLs', 'wordpress-social-login') ?></li>
					<li><?php __e('Added new providers: Amazon, Discord and Spotify to name a few', 'wordpress-social-login') ?></li>
					<li><?php __e('A number of providers are now defunct', 'wordpress-social-login') ?></li>
					<li><?php __e('Obligatory minor improvement', 'wordpress-social-login') ?></li>
					<li><?php __e('and whatnot.', 'wordpress-social-login') ?></li>
				</ul>
			</td>
		</tr>
...
function wsl_admin_help_us_localize_note()
{
	...
	?>
		<div id="l10n-footer">
			...
			<a href="https://www.transifex.com/projects/p/wordpress-social-login/" target="_blank"><?php __e( "Help us translate WordPress Social Login into your language", 'wordpress-social-login' ) ?></a>
		</div>
	<?php
}
