	...
	Postponed to future versions
		...
		<form method="post">
			<select name="provider" style="vertical-align: unset;">
				<option value=""><?php __e("Provider", 'wordpress-social-login') ?></option>
			</select>
			<input type="text" value="" name="username" placeholder="<?php __e("Username", 'wordpress-social-login') ?>" style="height: 28px;">
			<input type="submit" value="Filter" class="button">
		</form>
	-->

	<?php __e( "This screen only list the users who have connected through WordPress Social Login", 'wordpress-social-login' ) ?>.
</div>
<?php
...
<table cellspacing="0" class="wp-list-table widefat fixed users">
	<thead>
		<tr>
			<th width="100"><span><?php __e("Providers", 'wordpress-social-login') ?></span></th>
			<th><span><?php __e("Username", 'wordpress-social-login') ?></span></th>
			<th><span><?php __e("Full Name", 'wordpress-social-login') ?></span></th>
			<th><span><?php __e("E-mail", 'wordpress-social-login') ?></span></th>
			<th><span><?php __e("Profile URL", 'wordpress-social-login') ?></span></th>
			<th width="80"><span><?php __e("Contacts", 'wordpress-social-login') ?></span></th>
			<th width="55"><span><?php __e("User ID", 'wordpress-social-login') ?></span></th>
		</tr>
	</thead>
	<tfoot>
		<tr>
	...
	<tbody data-wp-lists="list:user" id="the-list">
		<?php
			...
			{
				?>
					<tr class="no-items"><td colspan="5" class="colspanchange"><?php __e("No users found", 'wordpress-social-login') ?>.</td></tr>
				<?php
			}
			else
				...
				<tr class="<?php if( ++$i % 2 ) echo "alternate" ?> tr-contacts">
					<td nowrap>
						<?php
							...
							{
								?>
									<img src="<?php echo $assets_base_url . strtolower( $link->provider ) . '.png' ?>" style="vertical-align:top;width:16px;height:16px;" /> <?php __e($link->provider, 'wordpress-social-login') ?><br />
								<?php
					...
					<td class="column-author">
						...
						<div class="row-actions">
							<span class="view">
								<a href="options-general.php?page=wordpress-social-login&wslp=users&uid=<?php echo $user_id ?>"><?php __e("Profiles", 'wordpress-social-login') ?></a>
								|
							</span>

							<span class="view">
								<a href="options-general.php?page=wordpress-social-login&wslp=contacts&uid=<?php echo $user_id ?>"><?php __e("Contacts", 'wordpress-social-login') ?></a>
								|
							</span>
							...
							<span class="delete">
								...
								?>
								<a style="color: #a00;" href="<?php echo $delete_url ?>" onClick="return confirmDeleteWSLUser();"><?php __e("Delete", 'wordpress-social-login') ?></a>
							</span>
						</div>
					</td>
