...
<div class="stuffbox">
	<h3>
		<label><?php __e("Basic Settings", 'wordpress-social-login') ?></label>
	</h3>
	<div class="inside">
		<p>
			<?php __e("<b>Connect with caption :</b> Change the content of the label to display above WSL widget", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Social icon set :</b> WSL provides two set of icons to display on the widget", 'wordpress-social-login') ?>.
			<?php __e("You can also display the providers names instead of icons. This allow the customization of the widget to a great extent", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Users avatars :</b> Determines whether to show users avatars from social networks or to display the default ones", 'wordpress-social-login') ?>.

			<?php __e("Avatars display should work right out of the box with most WordPress themes, BuddyPress and bbPress", 'wordpress-social-login') ?>.
		</p>

		<table width="100%" border="0" cellpadding="5" cellspacing="2" style="border-top:1px solid #ccc;">
		  <tr>
			<td width="180" align="right"><strong><?php __e("Connect with caption", 'wordpress-social-login') ?> :</strong></td>
			<td>
			<input type="text" class="inputgnrc" style="width:535px" value="<?php __e( get_option( 'wsl_settings_connect_with_label' ), 'wordpress-social-login' ); ?>" name="wsl_settings_connect_with_label" >
			</td>
		  </tr>
		  <tr>
			<td align="right"><strong><?php __e("Social icon set", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<?php
					$icon_sets = array(
				...
				<select name="wsl_settings_social_icon_set" style="width:535px">
					<?php
						...
						{
							?>
								<option <?php if( $wsl_settings_social_icon_set == $folder ) echo "selected"; ?>   value="<?php echo $folder; ?>"><?php __e( $label, 'wordpress-social-login' ) ?></option>
							<?php
						}
					?>
					<option <?php if( $wsl_settings_social_icon_set == "none" ) echo "selected"; ?>     value="none"><?php __e("None, display providers names instead of icons", 'wordpress-social-login') ?></option>
				</select>
			</td>
		  </tr>
		  <tr>
			<td align="right"><strong><?php __e("Users avatars", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<select name="wsl_settings_users_avatars" style="width:535px">
					<option <?php if( ! get_option( 'wsl_settings_users_avatars' ) ) echo "selected"; ?> value="0"><?php __e("Display the default WordPress avatars", 'wordpress-social-login') ?></option>
					<option <?php if(   get_option( 'wsl_settings_users_avatars' ) ) echo "selected"; ?> value="1"><?php __e("Display users avatars from social networks when available", 'wordpress-social-login') ?></option>
				</select>
			</td>
		  </tr>
		</table>
...
<div class="stuffbox">
	<h3>
		<label><?php __e("Advanced Settings", 'wordpress-social-login') ?></label>
	</h3>
	<div class="inside">
		<p>
			<?php __e("<b>Redirect URL :</b> By default and after they authenticate, users will be automatically redirected to the page where they come from. If WSL wasn't able to identify where they come from (or if they used wp-login page to connect), then they will be redirected to <code>Redirect URL</code> instead", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Force redirection :</b> When set to <b>Yes</b>, users will be <b>always</b> redirected to <code>Redirect URL</code>", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Authentication display :</b> Determines how the authentication dialog is rendered. You can chose to open the dialog in a <b>popup</b> or to <b>in page</b>. If a user is visiting using a mobile device, WSL will fall back to more <b>in page</b>", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Widget display :</b> Determines where you want to show the authentication widget", 'wordpress-social-login') ?>.
		</p>

		<p>
			<?php __e("<b>Notification :</b> Determines whether you want to receive a notification by mail when a new user is logged in via WSL", 'wordpress-social-login') ?>.
		</p>

		<table width="100%" border="0" cellpadding="5" cellspacing="2" style="border-top:1px solid #ccc;">
		  <tr>
			<td width="180" align="right"><strong><?php __e("Redirect URL", 'wordpress-social-login') ?> :</strong></td>
			<td>
		  ...
		  <tr>
			<td align="right"><strong><?php __e("Force redirection", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<select name="wsl_settings_force_redirect_url" style="width:100px">
					<option <?php if( get_option( 'wsl_settings_force_redirect_url' ) == 1 ) echo "selected"; ?> value="1"><?php __e("Yes", 'wordpress-social-login') ?></option>
					<option <?php if( get_option( 'wsl_settings_force_redirect_url' ) == 2 ) echo "selected"; ?> value="2"><?php __e("No", 'wordpress-social-login') ?></option>
				</select>
			</td>
		  </tr>
		  <tr>
			<td align="right"><strong><?php __e("Authentication display", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<select name="wsl_settings_use_popup" style="width:100px">
					<option <?php if( get_option( 'wsl_settings_use_popup' ) == 1 ) echo "selected"; ?> value="1"><?php __e("Popup", 'wordpress-social-login') ?></option>
					<option <?php if( get_option( 'wsl_settings_use_popup' ) == 2 ) echo "selected"; ?> value="2"><?php __e("In Page", 'wordpress-social-login') ?></option>
				</select>
			</td>
		  </tr>
		  <tr>
			<td align="right"><strong><?php __e("Widget display", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<?php
				...
				<select name="wsl_settings_widget_display" style="width:535px">
					<?php
						...
						{
							?>
								<option <?php if( $wsl_settings_widget_display == $display ) echo "selected"; ?>   value="<?php echo $display; ?>"><?php __e( $label, 'wordpress-social-login' ) ?></option>
							<?php
						}
					?>
				</select>
		  ...
		  <tr>
			<td align="right"><strong><?php __e("Notification", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<?php
				...
				<select name="wsl_settings_users_notification" style="width:535px">
					<option <?php if( $wsl_settings_users_notification == 0 ) echo "selected"; ?> value="0"><?php __e("No notification", 'wordpress-social-login') ?></option>
					<?php
						foreach( $users_notification as $type => $label )
						{
							?>
								<option <?php if( $wsl_settings_users_notification == $type ) echo "selected"; ?>   value="<?php echo $type; ?>"><?php __e( $label, 'wordpress-social-login' ) ?></option>
							<?php
						}
					?>
				</select>
...
<div class="stuffbox">
	<h3>
		<label><?php __e("Custom CSS", 'wordpress-social-login') ?></label>
	</h3>
	<div class="inside">
		<p>
			<?php __e("To customize the default widget styles you can either: change the css in the <b>text area</b> bellow or add it to your website <b>theme</b> files", 'wordpress-social-login') ?>.
		</p>
		...
		<p>
			<?php __e("The basic widget markup is the following", 'wordpress-social-login') ?>:
		</p>
