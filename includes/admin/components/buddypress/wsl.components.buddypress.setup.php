...
<div class="stuffbox">
	<h3>
		<label><?php __e("Users avatars", 'wordpress-social-login') ?></label>
	</h3>
	<div class="inside">
		<p>
			<?php
				if( get_option( 'wsl_settings_users_avatars' ) == 1 ){
					__e("<b>Users avatars</b> is currently set to: <b>Display users avatars from social networks when available</b>", 'wordpress-social-login');
				}
				else{
					__e("<b>Users avatars</b> is currently set to: <b>Display the default WordPress avatars</b>", 'wordpress-social-login');
				}
			?>.
		</p>

		<p class="description">
			<?php __e("To change this setting, go to <b>Widget</b> &gt; <b>Basic Settings</b> &gt <b>Users avatars</b>, then select the type of avatars that you want to display for your users", 'wordpress-social-login') ?>.
		</p>
	</div>
</div>
<?php
...
<div class="stuffbox">
	<h3>
		<label><?php __e("Profile mappings", 'wordpress-social-login') ?></label>
	</h3>
	<div class="inside">
		<p>
			<?php __e("When <b>Profile mapping</b> is enabled, WSL will try to automatically fill in Buddypress users profiles from their social networks profiles", 'wordpress-social-login') ?>.
		</p>
		...
		<p class="description">
			1. <?php __e('<b>Profile mapping</b> will only work for new users. Profile mapping for returning users will implemented in future version of WSL', 'wordpress-social-login') ?>.
			<br />
			2. <?php __e('Not all the mapped fields will be filled. Some providers and social networks do not give away many information about their users', 'wordpress-social-login') ?>.
			<br />
			3. <?php __e('WSL can only map <b>Single Fields</b>. Supported fields types are: Multi-line Text Areax, Text Box, URL, Date Selector and Number', 'wordpress-social-login') ?>.
		</p>

		<table width="100%" border="0" cellpadding="5" cellspacing="2" style="border-top:1px solid #ccc;">
		  <tr>
			<td width="200" align="right"><strong><?php __e("Enable profile mapping", 'wordpress-social-login') ?> :</strong></td>
			<td>
				<select name="wsl_settings_buddypress_enable_mapping" id="wsl_settings_buddypress_enable_mapping" style="width:100px" onChange="toggleMapDiv();">
					<option <?php if( $wsl_settings_buddypress_enable_mapping == 1 ) echo "selected"; ?> value="1"><?php __e("Yes", 'wordpress-social-login') ?></option>
					<option <?php if( $wsl_settings_buddypress_enable_mapping == 2 ) echo "selected"; ?> value="2"><?php __e("No", 'wordpress-social-login') ?></option>
				</select>
			</td>
		  </tr>
		</table>
...
<div id="xprofilemapdiv" class="stuffbox" style="<?php if( $wsl_settings_buddypress_enable_mapping == 2 ) echo "display:none;"; ?>">
	<h3>
		<label><?php __e("Fields Map", 'wordpress-social-login') ?></label>
	</h3>

	<div class="inside">
		<p>
			<?php __e("Here you can create a new map by placing WSL users profiles fields to the appropriate destination fields", 'wordpress-social-login') ?>.
			<?php __e('The left column shows the available <b>WSL users profiles fields</b>: These select boxes are called <b>source</b> fields', 'wordpress-social-login') ?>.
			<?php __e('The right column shows the list of <b>Buddypress profiles fields</b>: Those are the <b>destination</b> fields', 'wordpress-social-login') ?>.
			<?php __e('If you don\'t want to map a particular Buddypress field, then leave the source for that field blank', 'wordpress-social-login') ?>.
		</p>
										...
										<tr>
											...
											<td valign="top">
												...
												<?php
													...
													?>
														<p class="description">
															<?php __e("<b>WSL</b> can not map this field. Supported field types are: <em>Multi-line Text Areax, Text Box, URL, Date Selector and Number</em>", 'wordpress-social-login'); ?>.
														</p>
													<?php
													}
													else{
